...
- Long-running MCP work uses the durable task runtime in `app/mcp_tasks/service.py`:
  ...
  delivery, and tool-exposure rules live in the [runtime guide](packages/harness/deerflow/runtime/AGENTS.md).
- Cancellation ownership crosses MCP, RunManager, and journal boundaries. Each
  boundary currently uses a fixed internal five-second deadline. When
  ...
  continues. RunManager fencing is local; durable terminalization may be
  reconciled by peers/orphans. See the [runtime guide](packages/harness/deerflow/runtime/AGENTS.md)
