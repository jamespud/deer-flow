...
- Cancellation drains use one fixed 5.0-second monotonic deadline (`loop.time()`); repeated caller cancellation is absorbed without renewing that deadline. If a same-task operation remains ambiguous at the deadline, ownership transfers to one supervised background task which consumes its eventual outcome; no blind retry or requeue is allowed. RunManager applies its caller-provided shutdown hard total budget across cleanup producers, manager-lock waiters, active-run drain, heartbeat/orphan tasks, and trailing status persistence, with late outcomes consumed exactly once. MCP batch supervisors own the supervisor, every child, and every per-record release state: sibling releases start concurrently and each claimed record is released exactly once. Ordinary retry release and cancellation release are mutually exclusive, inner self-cancellation is isolated from the poller, and an outer cancellation releases started and never-started records without duplicating child cleanup. Journal predecessors are serialized before a later flush so an ambiguous `to_thread` or database write keeps ownership until its final result.
- The cancellation ownership handoff is explicit at each cross-component boundary: MCP claim/release handoff drains the durable claim result before releasing returned records; the RunManager finalization fence fences the local owner and marks ownership lost when needed, while durable terminalization authority remains with peer/orphan reconciliation; cancelled-admission cleanup attempts to terminalize and verify a replacement admission, logging a quick explicit failure while durable lease recovery remains the fallback; and the journal write handoff keeps an ambiguous event-store write owned until its result is known. Caller-cancellation drain timeouts transfer that same task to supervised background ownership and re-raise the original `CancelledError`; normal `McpTaskService.stop()` and `RunManager.shutdown()` deadlines record/log timeout state and return while supervised work continues. Cleanup failure cannot replace the caller's signal.
