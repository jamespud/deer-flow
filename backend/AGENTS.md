...
- The cancellation ownership handoff is explicit at each cross-component boundary: MCP claim/release handoff drains the durable claim result before releasing returned records; the RunManager finalization fence keeps terminal status and cleanup under the same owner; cancelled-admission cleanup closes a replacement admission before it can remain active; and the journal write handoff keeps an ambiguous event-store write owned until its result is known. Each timeout transfers that same task to supervised background ownership and re-raises the original `CancelledError`; cleanup failure cannot replace the caller's signal.
