...
- `migrations/versions/0017_personal_access_tokens.py` — adds the personal access token table and indexes; chains after `0016_subagent_batches`
- `migrations/versions/0018_mcp_task_lease_tokens.py` — chains after `0017_personal_access_tokens` and adds nullable `mcp_tasks.lease_token` / `notification_lease_token` columns so every poll, cancel, and notification mutation can be fenced to the exact claim generation
