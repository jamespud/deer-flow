...
- `migrations/versions/0017_mcp_task_lease_tokens.py` — chains after `0016_subagent_batches` and adds nullable `mcp_tasks.lease_token` / `notification_lease_token` columns so every poll, cancel, and notification mutation can be fenced to the exact claim generation
